...
election_list = [proxy2,proxy3,proxy4]
current_coordinator_port = server_port

# New variables
election_interval = 30      # Make a custom function to hold election
def initiateElection():     # called after a fixed interval via set_interval
# 1. sendRequest() to all higher servers proxy2, proxy3, proxy4.
# 2. Start a timer t1, to wait for the alive/co-ordinator's response.
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        for i, p in enumerate(election_list):
            try:
                init_time = time.time()
                print(f"Sending request to {[4000, 5000, 6000][i]}")
                # response = p.returnResponse()
                future = executor.submit(p.returnResponse)
                response = future.result()
                print(f"{p} response {response}")
                if int(time.time() - init_time) >= 5: # Timeout Case
                    flag = False
                    break
                pp = p
                flag = True
                break
            except: # p.returnResponse() error => Connection Refused Case
                continue
    if not flag:
        print(f"New Cooridnator is Server with port 3000, all other servers are down.")
        setCoordinator(server_port)
    else:
        print('Waiting...')
        pp.doElection()
    return
...
def setCoordinator(port):
    print('setCoordinator called for server 1...')
    global current_coordinator_port
    current_coordinator_port = port
    print('Set coordinator port as',port)

def sendRequest(proxy):
# 1. send request to the specified server instance, to check if alive
# 2. Ignore error if proxy doesn't connect [Wait for the end of t1]
    return proxy.returnResponse()
...
class Flight():
    ...
    def __str__(self):
        return f"['flight_number': {self.flight_number}, 'source': {self.source},'destination':
        {self.destination}, 'time_of_flight': {self.time_of_flight},'no_of_seats': {self.no_of_seats},
        'available_economy_seats':{self.available_economy_seats}, 'economy_price':
        {self.economy_price},'business_price': {self.business_price}, 'airline': {airline}]"
...
if __name__ == '__main__':
    try:
        ...
        set_interval(initiateElection, election_interval)
