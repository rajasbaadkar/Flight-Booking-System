...
def setCoordinator(port):
    print('setCoordinator called for server 3...')
...
def doElection():
    ...
    for i, p in enumerate(election_list):
        try:
            init_time = time.time()
            print(f"Sending request to {[6000][i]}")
    ...
    if not flag:
        for i, p in enumerate(proxy_list):
            print(f"Sending victory msg to {[3000, 4000][i]}")
            sendVictory(p)
    else:
        flag = False
        print('Waiting...')
