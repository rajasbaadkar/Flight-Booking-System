...
# Make a custom function to hold election
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        for i, p in enumerate(election_list):
            try:
                init_time = time.time()
                print(
                    f"Sending request to {[4000, 5000, 6000][i]}"
                ...
                continue
            # p.returnResponse() error => Connection Refused Case continue
    if not flag:
...
def serverSynchronize():
    global clock
    mutex.acquire()
    # Keep increasing at "rate", keeping in mind race around condition 
    clock += 1
    mutex.release()
...
def setServerClock(client_clock):
    # Server receives message from client global clock
    print(
        colored(
...
def requestCS(client_clock, clientID):
    ...
    if critical_mutex.locked():
        if clientID not in queue:
            ...
            print(
                colored(
                    f"Queue Mutex Acquired by {clientID}; queue:{queue}", "green")
            )
